...
namespace Project_Librari_Management.UI
{
    public partial class SummaryUI : Form
    {
        ...
        public void GetAll()
        {
           ...
           List<Book> sellBooks = new List<Book>();
            while(reader.Read())
            {
                ...
                double unitpricecell = Convert.ToDouble(reader[7]);
                double unitPrice = Convert.ToDouble(reader[8]);
                double total = Convert.ToDouble(reader[9]);
                string date = reader[10].ToString();
                Book aBook = new Book();
        ...
        private void SummaryUI_Load(object sender, EventArgs e)
        {
