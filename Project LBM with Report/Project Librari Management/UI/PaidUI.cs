    ...
    public partial class PaidUI : Form
    {
      public  RecordDue aDue = new RecordDue();
        ...
        public PaidUI(RecordDue aRecord)
        {
            ...
            previous_Due_textBox.Text = aDue.PreviousDue.ToString();
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
           
                 {
                      try
                      {
                          ...
                          d = (a + b + c)
                          ;
                          double collection = Convert.ToDouble(pBookTextBox.Text) + Convert.ToDouble(pPhotocopyTextBox.Text) + Convert.ToDouble(pOtheresTextBox.Text);
                          SqlConnection connection = manager.Connection();
                          string updateQuary = "Update DueRecords set F_Book='" + a + "',F_Copy='" + b + "',Mobile='"+ViewMobile.Text+"',F_Others='" + c + "',Total='" + d + "'where Serial ='" + aDue.Serial + "'";
                         ...
                         cmdCommand.ExecuteNonQuery();
                          ...
                          connection.Close();
                          string insert = "INsert INto Due_Collection values('" + collection + "','" + 0 + "','"+0+"','"+DateTime.Now.ToShortDateString()+"')";
                          SqlCommand cMdCommand = new SqlCommand(insert, connection);
                          connection.Open();
                          cMdCommand.ExecuteNonQuery();
                          connection.Close();
        ...
        private void addNeeDueBotton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SqlConnection connection = manager.Connection();
                string updateQuary = "Update DueRecords set F_Book='" + a + "',Mobile='" + ViewMobile.Text + "',F_Copy='" + b + "',F_Others='" + c + "',Total='" + d + "'where Serial ='" + aDue.Serial + "'";
                SqlCommand cmdCommand = new SqlCommand(updateQuary, connection);
                connection.Open();
                cmdCommand.ExecuteNonQuery();
                MessageBox.Show("Updated","Message",MessageBoxButtons.OK,MessageBoxIcon.Information);
                connection.Close();
                //string insertQuery = "INsert INto Due_Collection values(@collection,@)";
                string insert = "INsert INto Due_Collection values('" + 0 + "','" + y + "','"+0+"','"+DateTime.Now.ToShortDateString()+"')";
            ...
            catch (FormatException)
            {
                ...
                MessageBox.Show("Are you want to delivery this Book?", "Message", MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
        ...
        public void GetAllDueRecord()
        {
            try
            {
                DBManager manager1 = new DBManager();
                SqlConnection connection = manager1.Connection();
                string seletQuery = "select * from DueRecords where Serial='" + serialtextBox.Text+ "'";
                SqlCommand selectCmd = new SqlCommand(seletQuery, connection);
                connection.Open();
